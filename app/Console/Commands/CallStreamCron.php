...
class CallStreamCron extends Command
{
    ...
    public function handle(CallStreamService $callStream, CallStreamTokenService $callStreamToken)
    {
        ...
        // if (!$refresh_token) {
        //     Log::stack([$channel])->info('No token. Login get tokens.');
        ...
        //         Log::stack([$channel])->info('Create tokens.');
        //         if ($res) {
        //             Log::stack([$channel])->info('Create tokens success.');
        //         } else {
        //             Log::stack([$channel])->error('Error create tokens.');
        //         }
        //     } else {
        ...
        // $refresh_token_active = $refresh_token->expires_at > Carbon::now();

        // ? login and update token
        // if ($refresh_token && $refresh_token->token == null || $refresh_token && !$refresh_token_active) {
        //     if (!$refresh_token_active) {

        //         Log::stack([$channel])->info('Token lifetime expired. Login get tokens.');
        //     } else {

        //         Log::stack([$channel])->info('Token null. Get tokens.');
        //     }
        ...
        //     if ($tokens) {
        //         $res = $callStreamToken->updateTokens($tokens);
        //         Log::stack([$channel])->info('Update tokens.');
        //         if ($res) {
        //             Log::stack([$channel])->info('Update tokens success.');
        //         } else {
        //             Log::stack([$channel])->error('Error update tokens.');
